            ...
            {accounts.length > 0 ? (
                <>
                    <div style={{marginTop: '20px', marginBottom: '20px', textAlign: 'right'}}>
                        <Button onClick={() => history.push('/new-account')} color="primary" variant="contained" disabled><i className="fa fa-plus"></i> Add Account</Button>
                    </div>
                    ...
                    {accounts.map((account, i) => 
                        <Grid container item xs={12} md={3} key={i}>
                            <Card key={account.id} style={{width: '100%'}}>
                                ...
                                <CardActions>
                                    {(account.subscriptionStatus === 'active' || account.subscriptionStatus === 'paid' || account.subscriptionStatus === 'Completed')?(
